...
import { Play, Trophy, Zap, ChevronRight, Activity, Clock } from "lucide-react"
...
export function SportsSection() {
  ...
  const fetchMatches = async () => {
    ...
    try {
      ...
      // Filtramos para mostrar solo los 12 más importantes en el Home
      const topMatches = (json.matches || []).slice(0, 12)
  ...
  return (
    <section className="py-12 section-gradient overflow-hidden">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {/* HEADER UNIFICADO: Título y Categorías en una sola línea o bloque limpio */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
          <div>
            ...
            <p className="text-slate-400 text-sm mt-1">Selecciona un deporte y mira los eventos de hoy.</p>
          </div>

          {/* ÚNICO MENÚ DE CATEGORÍAS */}
        ...
        {/* 🚀 SCROLL LATERAL ESTILO APP PREMIUM */}
        {isLoading ? (
          <div className="flex gap-4 overflow-hidden">
            {[1, 2, 3, 4].map((i) => (
              <Skeleton key={i} className="min-w-[280px] md:min-w-[320px] h-48 rounded-3xl" />
            ))}
          </div>
        ) : matches.length > 0 ? (
          <div className="flex gap-4 overflow-x-auto pb-8 scrollbar-hide snap-x select-none">
            {matches.map((match) => (
              <div key={match.id} className="min-w-[280px] md:min-w-[320px] snap-start">
                <article className="glass border border-white/5 rounded-[2rem] p-6 hover:border-primary/40 transition-all group relative h-full flex flex-col justify-between">
                  ...
                  <div className="space-y-4 mb-6">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 bg-white/5 rounded-full p-1.5">
                  ...
                  {/* Botón de Acción */}
                  <Button 
                    className={`w-full rounded-2xl py-5 font-black text-[11px] tracking-widest uppercase transition-all ${
                      match.isLive 
                  ...
                  >
                    <Link href={`/partido/${match.slug}`}>
                      ...
                      {match.isLive ? 'Ver en la App' : 'Ver Horarios'}
                    </Link>
                  </Button>

                  {/* Click en toda la tarjeta para SEO */}
                  <Link href={`/partido/${match.slug}`} className="absolute inset-0 z-0 rounded-[2rem]">
                      <span className="sr-only">Ir a {match.slug}</span>
                  </Link>
                </article>
              </div>
            ))}
            
            {/* Tarjeta de "Ver más" al final */}
            <Link href="/agenda-deportiva" className="min-w-[180px] flex flex-col items-center justify-center glass border border-dashed border-white/10 rounded-[2rem] hover:bg-white/5 hover:border-primary/50 transition-all snap-start group">
               ...
               <span className="text-xs font-bold text-slate-300">Ver Agenda Completa</span>
            </Link>
          </div>
        ) : (
          <div className="text-center py-12 glass rounded-3xl border border-white/5">
            ...
            <p className="text-slate-400 font-bold">No hay eventos para esta categoría ahora.</p>
          </div>
        )}
